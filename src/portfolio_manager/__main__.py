...
from portfolio_manager.config import (
    github_repo_affiliation,
    github_repo_exclude_forks,
    llm_is_configured,
)
...
def main():
    ...
    if not llm_is_configured():
        log.error(
            "No LLM configured: set OPENAI_API_KEY, or OPENAI_BASE_URL for a local "
            "OpenAI-compatible server (see .env.example)."
        )
        sys.exit(1)
